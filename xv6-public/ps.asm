...
main(int argc, char *argv[]){
   ...
   d:	56                   	push   %esi
        ...
        if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
            ...
            state = "???"; 
   e:	be a8 07 00 00       	mov    $0x7a8,%esi
main(int argc, char *argv[]){
  13:	53                   	push   %ebx
  14:	bb e0 0b 00 00       	mov    $0xbe0,%ebx
  19:	51                   	push   %ecx
  1a:	83 ec 14             	sub    $0x14,%esp
    printf(1, "F    S    UID    PID    PPID    SZ    WCHAN    COMD\n");
  1d:	68 d8 07 00 00       	push   $0x7d8
  22:	6a 01                	push   $0x1
  ...
  2c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        if (p->state == UNUSED)
  30:	8b 43 a0             	mov    -0x60(%ebx),%eax
  33:	85 c0                	test   %eax,%eax
  35:	74 36                	je     6d <main+0x6d>
            state = "???"; 
  37:	ba a8 07 00 00       	mov    $0x7a8,%edx
        if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
  3c:	83 f8 05             	cmp    $0x5,%eax
  3f:	77 0c                	ja     4d <main+0x4d>
  41:	8b 14 85 40 08 00 00 	mov    0x840(,%eax,4),%edx
            state = "???"; 
  48:	85 d2                	test   %edx,%edx
  4a:	0f 44 d6             	cmove  %esi,%edx
       printf(1, "2    %s    Root    %d    %d    %d    %d    %s\n", state, p->pid, p->parent->pid,p->sz,p->chan, p->name);
  4d:	8b 43 a8             	mov    -0x58(%ebx),%eax
  50:	53                   	push   %ebx
  51:	ff 73 b4             	pushl  -0x4c(%ebx)
  54:	ff 73 94             	pushl  -0x6c(%ebx)
  57:	ff 70 10             	pushl  0x10(%eax)
  5a:	ff 73 a4             	pushl  -0x5c(%ebx)
  5d:	52                   	push   %edx
  5e:	68 10 08 00 00       	push   $0x810
  63:	6a 01                	push   $0x1
  65:	e8 d6 03 00 00       	call   440 <printf>
  6a:	83 c4 20             	add    $0x20,%esp
  6d:	83 c3 7c             	add    $0x7c,%ebx
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
  70:	81 fb e0 2a 00 00    	cmp    $0x2ae0,%ebx
  76:	75 b8                	jne    30 <main+0x30>
    }
    exit();
  78:	e8 54 02 00 00       	call   2d1 <exit>
  7d:	66 90                	xchg   %ax,%ax
  7f:	90                   	nop
 ...
 386:	89 d3                	mov    %edx,%ebx
{
 ...
 3c9:	0f b6 92 60 08 00 00 	movzbl 0x860(%edx),%edx
...
printf(int fd, const char *fmt, ...)
{
 ...
 5c4:	eb 1a                	jmp    5e0 <printf+0x1a0>
          s = "(null)";
 5c6:	bb 58 08 00 00       	mov    $0x858,%ebx
        while(*s != 0){
...
free(void *ap)
{
 ...
 611:	a1 20 0b 00 00       	mov    0xb20,%eax
{
 ...
 636:	73 10                	jae    648 <free+0x38>
{
 ...
 661:	5b                   	pop    %ebx
  freep = p;
 662:	a3 20 0b 00 00       	mov    %eax,0xb20
}
 667:	5e                   	pop    %esi
 668:	5f                   	pop    %edi
 669:	5d                   	pop    %ebp
 ...
 687:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 68a:	a3 20 0b 00 00       	mov    %eax,0xb20
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 6a9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6ac:	8b 3d 20 0b 00 00    	mov    0xb20,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6f9:	8b 3d 20 0b 00 00    	mov    0xb20,%edi
 ...
 722:	e8 e9 fe ff ff       	call   610 <free>
  return freep;
 727:	8b 15 20 0b 00 00    	mov    0xb20,%edx
 ...
 754:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 757:	89 15 20 0b 00 00    	mov    %edx,0xb20
}
 ...
 76f:	90                   	nop
    base.s.ptr = freep = prevp = &base;
 770:	c7 05 20 0b 00 00 24 	movl   $0xb24,0xb20
 777:	0b 00 00 
    base.s.size = 0;
 77a:	bf 24 0b 00 00       	mov    $0xb24,%edi
    base.s.ptr = freep = prevp = &base;
 77f:	c7 05 24 0b 00 00 24 	movl   $0xb24,0xb24
 786:	0b 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 789:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 78b:	c7 05 28 0b 00 00 00 	movl   $0x0,0xb28
 792:	00 00 00 
    if(p->s.size >= nunits){
